...
parser.add_argument(
    "-dp", "--data_path", type=str, default="./data/", help="Path to the FSC147 dataset"
)
parser.add_argument(
    "-ts",
    "--test_split",
...
if not exists(anno_file) or not exists(im_dir):
    ...
    print(
        "Current setting is {}, but the image dir and annotation file do not exist.".format(
            args.data_path
        )
    )
...
for im_id in pbar:
    ...
    with torch.no_grad():
        ...
        features = extract_features(
            resnet50_conv, image.unsqueeze(0), boxes.unsqueeze(0), MAPS, Scales
        )
    ...
    SAE += err
    SSE += err ** 2
    save_path = data_path + "/output/" + im_id + ".png"
    visualize_output_and_save(features, output, boxes, save_path)

    pbar.set_description(
        "{:<8}: actual-predicted: {:6d}, {:6.1f}, error: {:6.1f}. Current MAE: {:5.2f}, RMSE: {:5.2f}".format(
            im_id,
            gt_cnt,
            pred_cnt,
            abs(pred_cnt - gt_cnt),
...
print(
    "On {} data, MAE: {:6.2f}, RMSE: {:6.2f}".format(
